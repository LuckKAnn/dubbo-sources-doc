...
public class HeaderExchangeHandler implements ChannelHandlerDelegate {
    ...
    void handleRequest(final ExchangeChannel channel, Request req) throws RemotingException {
        ...
        try {
            /**
             * 这里的代码利用了CompletionStage，应该是类似于ComletableFuture，异步机制
             */
            // 继续向下调用，分异步调用和同步调用，如果是同步则会阻塞，如果是异步则不会阻塞
            //这一步会去到DubboProtocol的reply里面，拿到invoker
            ...
            future.whenComplete((appResult, t) -> {
                try {
                    if (t == null) {
                        ...
                        //在这种远程服务调用里面，异常应该报到服务调用这那里去，而不是在服务提供者这里
                        res.setStatus(Response.SERVICE_ERROR);
                        res.setErrorMessage(StringUtils.toString(t));
                    }
                    //在返回之前还会经过ProtocolFilterWrapper（这好像是dubbo的机制）
                    channel.send(res);
